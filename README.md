...
举例：全国 `AI产品经理` 任务采用合格数量门禁：每个候选必须取得完整 JD，并在现有一次 AI 请求中同时返回 `is_ai_operations`、岗位职责、任职要求和加分项。只有 `crawl_status='completed' AND ai_status='completed'` 才计入目标 50；销售 AI 产品、CIO、纯产品经理、人工智能训练师或只在要求/公司介绍中提到 AI 的岗位标记为 `irrelevant`，保留用于去重但不重试、不导出、不算失败。页面耗尽或无新增且不足 50 时进入 `incomplete`，不会伪装成完成。该任务把每个已保存的完整 JD 交给单 AI Worker；详情抓取本身保持串行，但可在 AI 处理上一条 JD 时抓取下一条详情，AI 调用不会并发扩张。15 页 / 450 个候选只是单城扫描上限，不是合格岗位目标；页面耗尽后任务正常完成，不会为凑满 450 个合格岗位继续补采。
