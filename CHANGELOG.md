...
### 修复
- 详情页不再仅因 URL 含 `_security_check` 或未来未知标记而误判访问受限；无明确受限证据且岗位身份、完整 JD 有效时正常保存。页面状态改为按 JD 外最小可见文本单元逐项判断，并识别可见验证码 iframe 的明确元数据；检测不依赖节点命名、不跨父子或页面区域拼词，隐藏模板与普通“安全校验”“滑块”“验证”“数据库访问频繁”等业务文案不再触发页面风控判定
