...
### 变更
- Streamlit 新建任务改为任意目标岗位的完整 JD 精准审核，只让用户填写目标岗位数量（最多 450）；移除开发阶段遗留的固定数量模式和页数输入，内部保留 15 页、每页 30 个候选的硬上限
- 历史任务与 `run_mode` 数据保持兼容；旧任务和当前 worker 不迁移、不重启
...
### 修复
- 详情页不再仅因 URL 含 `_security_check` 或未来未知标记而误判访问受限；无明确受限证据且岗位身份、完整 JD 有效时正常保存。页面状态改为按 JD 外最小可见文本单元逐项判断，并识别可见验证码 iframe 的明确元数据；单个明确 overlay 作为完整状态单元，除此之外不跨父子候选或无关页面区域拼词。检测不依赖节点命名，隐藏模板与普通“安全校验”“滑块”“验证”“数据库访问频繁”等业务文案不再触发页面风控判定
